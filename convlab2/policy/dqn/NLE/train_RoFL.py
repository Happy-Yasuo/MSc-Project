...
def fine_tune(pos_action, neg_action, tokenizer, model, data_set):
    ...
    save_dir = os.path.join(root_dir, 'convlab2/policy/dqn/NLE/save/' + data_set + '_fine_tune')
    log_dir = os.path.join(root_dir, 'convlab2/policy/dqn/NLE/save/' + data_set + '_fine_tune/logs')
...
def pretrain(env, expert_policy, policy, batchsz, tokenizer, data_set):
    """
    ...
    while len(prefill_buff.expert_demo) < pre_train_frames_num:
        ...
        if fine_tune_cnt % 6 == 5:
            fine_tune(pos_fine_tune_buff, neg_fine_tune_buff, tokenizer, expert_policy.model, data_set)
            fine_tune_checkpoint = os.path.join(root_dir, 'convlab2/policy/dqn/NLE/save/' + data_set +
                                                '_fine_tune/fine_tune_checkpoint')
...
def train_update(prefill_buff, env, policy, batchsz, epoch, data_set):
    seed = epoch
    random.seed(seed)
    np.random.seed(seed)
    torch.manual_seed(seed)
    ...
    if epoch % 10 == 0:
        # save current model
        policy.save(os.path.join(root_dir, 'convlab2/policy/dqn/NLE/save/' + data_set + '_fine_tune'), epoch)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--data_set", type=str, default="personachat", help="training data domain")
    ...
    # pre-train
    prefill_buff = pretrain(env, expert_policy, policy_sys, args.batchsz, tokenizer, args.data_set)
    prefill_buff.max_size = 100000

    for i in range(args.epoch):
        train_update(prefill_buff, env, policy_sys, args.batchsz, i, args.data_set)
